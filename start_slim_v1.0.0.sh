...
Help()
{
    ...
    echo
    echo "Syntax: start_slim.sh [-h] [-d] [-P port] [-C dns_name] [-S] [-K kraken2_db]"
    echo "options:"
    echo "-h --help                 Print this help."
    echo "-d --docker               Use docker instead of podman."
    echo "-P --port                 [bind-ip:]host:container port mapping. Default: 8080:80"
    echo "                           Example for reverse proxy HTTPS: 127.0.0.1:8080:80"
    echo "-C --caddy-domain         Generate an ignored Caddyfile for this DNS name and bind SLIM to localhost."
    echo "--caddyfile               Path for the generated Caddyfile. Default: deployment/Caddyfile"
...
slim_url_from_port_mapping()
{
    ...
    printf "http://%s:%s" "${host}" "${host_port}"
}

port_mapping_host_port()
{
    local mapping="$1"

    IFS=':' read -r -a port_parts <<< "${mapping}"
    if [ "${#port_parts[@]}" -eq 3 ]; then
        printf "%s" "${port_parts[1]}"
    else
        printf "%s" "${port_parts[0]}"
    fi
}
...
write_caddyfile()
{
    ...
    host_port="$(port_mapping_host_port "${mapping}")"
    if ! mkdir -p "$(dirname "${path}")"; then
        echo "Error: could not create Caddyfile directory for ${path}"
        exit 1
    fi

    if ! cat > "${path}" <<EOF
# Generated by start_slim_v1.0.0.sh.
# This file contains deployment-specific information and should not be committed.
${domain} {
	reverse_proxy 127.0.0.1:${host_port}
}
EOF
    then
        echo "Error: could not write Caddyfile to ${path}"
        exit 1
    fi

    echo "Generated Caddyfile: ${path}"
    echo "Note: start_slim only writes the Caddyfile; it does not install, start, or reload Caddy."
    echo "To activate HTTPS with Caddy:"
    echo "  sudo cp ${path} /etc/caddy/Caddyfile"
    echo "  sudo caddy fmt --overwrite /etc/caddy/Caddyfile"
    echo "  sudo systemctl reload caddy"
    echo "  sudo systemctl status caddy"
    echo "Then open: https://${domain}/"
    echo "If http://${domain}/ shows nothing, Caddy or public port 80 is not reachable."
    echo "Do not open http://${domain}:${host_port}/; that bypasses Caddy and stays unsecured."
}
...
while [ "$#" -gt 0 ]; do
    case "$1" in
        ...
        -C|--caddy-domain)
            if [ "$#" -lt 2 ]; then
                echo "Error: -C/--caddy-domain requires a DNS name."
                Help
                exit 1
            fi
            caddy_domain="$2"
        ...
        --caddyfile)
            if [ "$#" -lt 2 ]; then
                echo "Error: --caddyfile requires a path."
                Help
                exit 1
            fi
            CADDYFILE_PATH="$2"
...
if [[ "${port}" != *":"* ]]; then
    echo "Error: port option must follow the syntax [bind-ip:]host:container, e.g. 8080:80 or 127.0.0.1:8080:80"
    exit 1
fi

IFS=':' read -r -a initial_port_parts <<< "${port}"
if [ "${#initial_port_parts[@]}" -ne 2 ] && [ "${#initial_port_parts[@]}" -ne 3 ]; then
    echo "Error: port option must follow the syntax [bind-ip:]host:container, e.g. 8080:80 or 127.0.0.1:8080:80"
    exit 1
fi

if [ -n "${caddy_domain}" ]; then
    if [[ ! "${caddy_domain}" =~ ^[A-Za-z0-9][A-Za-z0-9.-]*[A-Za-z0-9]$ ]]; then
        echo "Error: Caddy DNS name contains unsupported characters: ${caddy_domain}"
        exit 1
    fi

    if [ "${port_was_set}" = "no" ]; then
        port="127.0.0.1:$(port_mapping_host_port "${port}"):$(port_mapping_container_port "${port}")"
    else
        IFS=':' read -r -a port_parts <<< "${port}"
        if [ "${#port_parts[@]}" -eq 2 ]; then
            port="127.0.0.1:${port_parts[0]}:${port_parts[1]}"
        elif [ "${#port_parts[@]}" -eq 3 ] && [ "${port_parts[0]}" != "127.0.0.1" ] && [ "${port_parts[0]}" != "localhost" ]; then
            echo "Warning: --caddy-domain keeps SLIM private; replacing bind address ${port_parts[0]} with 127.0.0.1."
            port="127.0.0.1:${port_parts[1]}:${port_parts[2]}"
        fi
    fi
fi
...
if [ -n "${caddy_domain}" ]; then
    echo "HTTPS DNS name: ${caddy_domain}"
    write_caddyfile "${caddy_domain}" "${port}" "${CADDYFILE_PATH}"
fi
...
echo "SLIM is running at $(slim_url_from_port_mapping "${port}")"
if [ -n "${caddy_domain}" ]; then
    echo "Internal SLIM URL for Caddy: http://127.0.0.1:$(port_mapping_host_port "${port}")"
    echo "Public HTTPS URL after Caddy is reloaded: https://${caddy_domain}/"
    echo "Plain http://${caddy_domain}/ should redirect to HTTPS only after Caddy is reachable on public port 80."
    echo "If you use http://${caddy_domain}:$(port_mapping_host_port "${port}")/, you are bypassing Caddy and Chrome will report an unsecured site."
fi
