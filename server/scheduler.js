...
var remove_outputs = function (params, token) {
	...
	for (let key in params) {
		...
		// wildcard-creator outputs are aliases for existing uploaded files, not
		// files generated by the module. Do not delete matching FASTQs here.
		if (soft.name == 'wildcard-creator') {
			continue;
		}
...
var computeSoftwareOrder = function (params, token) {
	...
	var addExecutableSoftwares = function () {
		...
		for (let soft_idx in params) {
			...
			for (var idx in soft.params.inputs) {
				var file = soft.params.inputs[idx];
				file = file.replace('$', '*');
				file = file.replace('€', '*');
				if (dependencies[file] != undefined) {
					isExecutable = false;
			...
			if (isExecutable && order.indexOf(soft.params.id) == -1) {
				...
				for (var id in soft.params.outputs) {
					var out_file = soft.params.outputs[id];
					filesAvailable.push(out_file);
				}
			}
		}
	};
	...
	// Queue tools with no unsatisfied dependencies, such as wildcard-creator.
	addExecutableSoftwares();
